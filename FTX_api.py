...
class FtxClient:
    ...
    def get_all_trades(self, market: str, start_time: float = None, end_time: float = None) -> List:
        ...
        while True:
            ...
            if len(response) == 0:
                break
            end_time = min(parse_date(t['time']) for t in response).timestamp()
    ...
    def post_quantzone_expression(self, expression: str = None) -> dict:
        return self._post('quantzone/test_expression',{'expression': expression})